...
MS-06 estabilizou os achados do QA de produção sem redesign. A causa do overflow após rotação era a largura inline obsoleta do único `pin-spacer` e da Hero; a trilha mobile de Formações já continha corretamente seu próprio overflow. O refresh agora observa a geometria real da viewport, reavalia `gsap.matchMedia` após estabilização e mantém pin/hero fluidos. Dezoito transições consecutivas, incluindo 844×390 ↔ 390×844 e 1024×768 → 768×1024 → 430×932 → 1280×720, mantiveram largura do documento igual à viewport e exatamente um pin em movimento normal. Tutores perdeu o `aria-label` proibido da `div.tutors-conversation`; `ETAPA` e os metadados orbitais/mobile receberam contraste pontual AA. Em Chrome headless com `prefers-reduced-motion: reduce` forçado, 1440×900, 1024×768, 390×844 e 844×390 apresentaram zero pins, Hero em fluxo relativo, órbitas/ticker/cursor sem loop, reveals visíveis e Companies com scroll manual. Lighthouse 13.0.1 no build local corrigido registrou acessibilidade 100 em mobile e desktop; a nova aferição no deploy atualizado, zoom e dispositivo real permanecem pendentes.
